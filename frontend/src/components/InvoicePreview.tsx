		...
		>
			...
			<div className="flex border-b border-black">
				...
				<div className="w-[180px] border-l border-black p-2 bg-gray-50 flex flex-col justify-between">
					<h2 className="text-xl font-bold text-center border-b border-black pb-1 mb-1 uppercase">Tax Invoice</h2>
			...
			<div className="flex border-b border-black">
				<div className="flex-1 p-2 border-r border-black">
					...
					<div className="px-1 space-y-0.5 font-semibold">
						<p className="text-sm font-bold uppercase">{invoice.customerName}</p>
						<p className="text-gray-700">{invoice.customerAddress}</p>
						<div className="flex gap-4 mt-1">
							<p><span className="font-bold">GSTIN:</span> {invoice.customerGstin || '-'}</p>
				...
				<table className="w-full border-collapse">
					...
					<tbody>
							...
							return (
								<tr key={idx} className="text-center font-medium">
									...
									<td className="border border-black p-1 text-left">
										<p className="font-bold uppercase">{item.itemName}</p>
										{item.description && <p className="text-[9px] text-gray-600 font-normal">{item.description}</p>}
									</td>
									<td className="border border-black p-1 text-gray-500 italic">Gen-Item</td>
						...
						<tr className="font-bold bg-gray-50 uppercase text-[10px]">
							...
							<td className="border border-black p-1 text-right">{invoice.subtotal.toFixed(2)}</td>
							<td className="border border-black p-1"></td>
							<td className="border border-black p-1 text-right">{invoice.grandTotal.toFixed(2)}</td>
						</tr>
					</tbody>
				</table>
			</div>
			...
			<div className="flex border-x border-b border-black">
				...
				<div className="w-[300px]">
					<div className="grid grid-cols-[1fr_100px] border-b border-black font-bold text-[10px]">
						...
						<div className="p-1 border-l border-black text-right">₹{invoice.subtotal.toFixed(2)}</div>
					</div>
					<div className="grid grid-cols-[1fr_100px] border-b border-black font-bold text-[10px]">
						<div className="p-1 uppercase text-red-600">Total Discount:</div>
						<div className="p-1 border-l border-black text-right text-red-600">- ₹{invoice.totalDiscount.toFixed(2)}</div>
					</div>
					<div className="grid grid-cols-[1fr_100px] border-b border-black font-bold text-[10px]">
						<div className="p-1 uppercase text-gray-700">Total GST:</div>
						<div className="p-1 border-l border-black text-right">₹{invoice.totalGST.toFixed(2)}</div>
					</div>
					<div className="grid grid-cols-[1fr_100px] bg-gray-50 border-b border-black font-bold text-xs">
						<div className="p-1.5 uppercase">GRAND TOTAL:</div>
						<div className="p-1.5 border-l border-black text-right text-blue-800">₹{invoice.grandTotal.toFixed(2)}</div>
					</div>
				</div>
			</div>
			...
			<div className="border border-black mt-2 p-2 relative">
				...
				<p className="font-bold italic text-gray-700">{numberToWords(invoice.grandTotal)}</p>
			</div>
			...
			<div className="border border-black mt-2 min-h-12 relative p-2">
				...
				<p className="text-gray-700 italic">{invoice.notes || 'No remarks provided'}</p>
			</div>
